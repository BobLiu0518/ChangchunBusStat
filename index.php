...
foreach($data['vehicleList'] as $vehicle){
    ...
    $csv .= "\n".implode(',', [$vehicle['busLicense'], completeRouteName($vehicle['lastOnLine']), $vehicle['busId'],
        $vehicle['onLine'][count($vehicle['onLine']) - 1]['lastFound'], implode('/', array_unique($allLines))]);
}
